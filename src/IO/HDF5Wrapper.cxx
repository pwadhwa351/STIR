...
#if 0
Succeeded HDF5Wrapper::initialise_geo_scaling_factors(const std::string& path)
{
    if(path.size() == 0)
    {
        if(is_signa)
        {
            m_address = "/ScaleFactors/Segment4/scaleFactors";
            std::ostringstream datasetname;
            m_dataset_sptr.reset(new H5::DataSet(file.openDataSet(m_address)));
            m_dataspace = m_dataset_sptr->getSpace();
            // PW here I output the dataspace dimensions and order to be correctly translated in the main code.
            ...
            //! \todo Get these numbers from the HDF5 file
            {
                m_NX_SUB = 16;    // hyperslab dimensions
                m_NX = 16;        // output buffer dimensions
            }
        }
        else
            return Succeeded::no;
...
#if 0
Succeeded HDF5Wrapper::get_from_1d_dataset(const std::array<unsigned long long int, 1>& offset,
